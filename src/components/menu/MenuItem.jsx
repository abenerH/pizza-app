...
const MenuItem = ({product, onItemClicked}) => {
  ...
  const middle = `$${(product.price + product.price * 0.15).toFixed(2)}`;
  const jumbo = `$${(product.price + product.price * 0.30).toFixed(2)}`;
  ...
  return (
    <Card className={'bg-slate-800 rounded-xl flex overflow-hidden text-white max-h-[250px] cursor-pointer hover:bg-slate-600 p-6 lg:py-0 lg:px-4'} onclick={itemClickedHandler}>
        ...
        <div className='w-3/4 self-center'>
            ...
            <p className='text-sm lg:text-xl mb-4'>{product.description}</p>
            <div className='flex justify-evenly'>
              <p className='font-bold lg:text-2xl bg-cyan-400 p-1 rounded-lg'>{small}</p>
              <p className='font-bold lg:text-2xl bg-green-400 p-1 rounded-lg'>{middle}</p>
              <p className='font-bold lg:text-2xl bg-red-400 p-1 rounded-lg'>{jumbo}</p>
